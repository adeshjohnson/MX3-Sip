...
=end
  ...
  def Device.find_all_for_select(current_user_id = nil, options={})
    ...
    if current_user_id
      return Device.select("devices.id, devices.description, devices.extension, devices.device_type, devices.istrunk, devices.name, devices.ani, devices.username").joins("LEFT JOIN users ON (users.id = devices.user_id) #{jn}").where(["device_type != 'FAX' AND (users.owner_id = ? OR users.id = ?) AND devices.name not like 'mor_server_%' AND user_id > -1", current_user_id, current_user_id]).all
    else
