      ...
      payment.save
	    if payment
		   ...
		   if !self.address.mob_phone.blank?
				...
				message = _('Body_message_new_payment', payment.paymenttype.to_s, payment.amount.to_s, self.username.to_s, self.balance.to_s)
				sms = SmsMessage.new
				sms.sending_date = Time.now
				sms.user_id = @user1.id
				sms.reseller_id = @user1.owner_id
				sms.number = self.address.mob_phone
				sms.sms_type = "payment"
				sms.reg_owner = self.owner_id
				sms.save
		  ...
		  if !self.email.blank?	
			...
			begin		
				  ...
				  Action.add_action(self.id, "Initial_balance", "Email not sent to user #{self.username} after payment: #{payment.amount} #{payment.currency}")
			end
		  else
				 Action.add_action(self.id, "Initial_balance", "User #{self.username} was not emailed after payment: #{payment.amount} #{payment.currency}. Email for is not registered.")
		  end
	    end
    end
  end
...
=end
  ...
  def User.create_from_registration(params, owner, reg_ip, free_ext, pin, pasw, nan, api=0)
    ...
    user.usertype = "user"
    user.first_name = params[:first_name] if params[:first_name].to_s != ""
    user.last_name = params[:last_name] if params[:last_name].to_s != ""
	...
	if api == 1
		user.balance = owner.to_system_currency(user.balance)	
	else
	...
	val_hash = nil
	if Confline.get_value('Default_User_blocked', owner.id).to_i == 1
		user.validation_hash = Digest::SHA1.hexdigest(user.id.to_s + Time.now.to_s)
		user.language.blank? ? lang = '&lang=es' : lang = '&lang=' + user.language
		url_hash = Web_URL + Web_Dir + '/callc/email_validation?val_id=' + user.id.to_s + '&val_hash=' + user.validation_hash + lang
		val_hash = _('Complete_Email_Validation') + ': ' + url_hash
	end
	
    user.save

    cb = (defined?(CALLB_Active) and (CALLB_Active == 1)) ? 1 : 0  #Hereee....
	#Adesh
	cli_mob_phone = "+" + params[:mob_phone].to_s
    ncli1 = Callerid.where("cli = '#{cli_mob_phone}'").first if params[:mob_phone].to_s.gsub(/[^0-9]/, "").length > 0
	if ncli1.blank? and params[:mob_phone].to_s.gsub(/[^0-9]/, "").length > 0
      cli = Callerid.new({:cli => cli_mob_phone, :device_id => device.id, :description => "Mobile Phone", :email_callback => cb, :added_at => Time.now})
      cli.save
    end
	ncli2 = Callerid.where("cli = '#{params[:phone]}'").first  if params[:phone].to_s.gsub(/[^0-9]/, "").length > 0
    if ncli2.blank? and params[:phone].to_s.gsub(/[^0-9]/, "").length > 0
	...
	ncli3 = Callerid.where("cli = '#{params[:fax]}'").first  if params[:fax].to_s.gsub(/[^0-9]/, "").length > 0
    if ncli3.blank? and params[:fax].to_s.gsub(/[^0-9]/, "").length > 0
      cli = Callerid.new({:cli => params[:fax].to_s.gsub(/[^0-9]/, ""), :device_id => device.id, :description => "Fax", :email_callback => cb, :added_at => Time.now})
      cli.save
    end
		
	send_email_to_user = nil
	notice = nil
    if !user.email.blank?
		begin	
			if api.to_i == 1
				a = Thread.new {
					send_email_to_user = EmailsController.send_user_email_after_registration(user, device, params[:password], reg_ip, free_ext, val_hash)
  ...
  def User.validate_from_registration(params, owner_id = 0, mobile)
	...
	if mobile == 1	
		if params[:mob_phone_orig].blank? and notice.blank?
			notice = _('Please_enter_mobile_phone')
		end
		if params[:verif_code].blank? and notice.blank?
			notice = _('Please_enter_verification_code')
		end		
		if notice.blank?			
      ...
      usuarios = User.where("username = '#{params[:mob_phone]}'").first #The mob_phone is the same user.
			...
			if notice.blank?
				sms = SmsMessage.where("number = '#{params[:mob_phone]}' AND verification_code = '#{params[:verif_code]}' AND sms_type = 'verification' ").first
				if 	!sms
					notice = _('Validation_of_mobile_phone_failed')
				end
			end
		end		
      ...
      #end
    #end
	params[:device_type] = 'SIP' if params[:device_type] != ""
  ...
  def update_from_edit(params, current_user, tax_from_params, monitoring_a, rec_a, api = 0)
	...
	# START Added by Adesh
	if api == 1
      self.send_balance_to_others = params[:send_balance_to_others].to_i if params[:send_balance_to_others]
    else
      self.send_balance_to_others= params[:send_balance_to_others].to_i
    end
	#END Added by Adesh
  ...
  def warning_email_hour= value
    if value.to_i != -1
      Time.zone	   = self.time_zone
      session_time = Time.zone.now.change(hour: value.to_i, min: 0, sec: 0)
      Time.zone	   = User.find(0).time_zone
      c = Time.zone.parse(session_time.to_s).hour
    else
      c = value
    end
  ...
  def validate_from_update(current_user, params, allow_edit, api = 0)
	...
	#Adesh
	if params[:user][:commission_percentage]
      params[:user][:commission_percentage]=params[:user][:commission_percentage].strip
      if params[:user][:commission_percentage].to_d < 0
        params[:user][:commission_percentage] = 0
      end
    end
	#Adesh
	...
	#Adesh
    ['tax2_enabled', 'tax3_enabled', 'tax4_enabled', 'own_providers', 'recording_enabled', 'recording_forced_enabled', 'compound_tax', 'show_zero_calls', 'unlimited', 'ignore_global_monitorings', 'block_conditional_use', 'warning_email_active', 'send_balance_to_others'].each { |p|
