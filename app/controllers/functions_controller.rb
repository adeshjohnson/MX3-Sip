...
class FunctionsController < ApplicationController
  ...
  def send_balance
	@page_title = _('Send_balance')

    @user = current_user

    if !@user.send_balance_to_others?
      flash[:notice] = _('Not_allowed_to_send_balance')
      redirect_to :controller => "callc", :action => 'main' and return false
    end
		
	not_commission_index = (100 - @user.commission_percentage )/100
	@minimal_amount_to_transfer = 0.2 * count_exchange_rate(session[:default_currency], @user.currency)
	unless @user.raw_balance  > (0.2 * not_commission_index)
		flash[:notice] = _('Your_balance_must_be_greater_than') + " " + (0.2 * not_commission_index * count_exchange_rate(session[:default_currency], @user.currency)).to_s
		redirect_to :controller => 'callc', :action => 'main' and return false
	end
  ...
  def send_balance_to
	...
	unless @user.send_balance_to_others != 0
		flash[:notice] = _('You_are_not_allowed_to_send_balance')
		redirect_to :controller => "callc", :action => 'main' and return false
	end
	
	unless !params[:receiver_type].blank? and !params[:receiver_name].blank? and !params[:amount].blank?
		flash[:notice] = _('Complete_all_necessary_data')
		redirect_to :action => 'send_balance' and return false
	end
	
	@amount = params[:amount].to_d 
	amount_default_currency = @amount * count_exchange_rate(@user.currency, session[:default_currency])

	unless amount_default_currency > 0.2
		flash[:notice] = _('Amount_must_be_a_number_greater_than') + " " + (0.2 * count_exchange_rate(session[:default_currency], @user.currency)).to_s
		redirect_to :action => 'send_balance' and return false
	end
	
	@amount_to_extract = @amount * ( 100 - @user.commission_percentage )/100
	
	if @user.balance < @amount_to_extract
      flash[:notice] = _('Not_enough_amount_in_own_balance') + ": " + @user.balance.to_d.to_s + " menor que " + @amount_to_extract.to_s
      redirect_to :action => 'send_balance' and return false
    end
	
	if params[:receiver_type].to_i == 0
		@user2 = User.where(:username => params[:receiver_name].to_s).first
	else
		@device2 = Device.where(:username => params[:receiver_name].to_s).first
		@user2 = @device2.user if @device2
	end
		
	unless @user2
    	flash[:notice] = _('The_user_was_not_found')
		redirect_to :action => 'send_balance' and return false	
	end
	
	unless @user2.owner_id != @user.id
		flash[:notice] = _('Invalid_this_user_belongs_to_you')
		redirect_to :action => 'send_balance' and return false		
	end
	
	unless @user2.username != @user.username
		flash[:notice] = _('The_user_is_yourself')
		redirect_to :action => 'send_balance' and return false	
	end

    @user.balance -= @amount_to_extract	
	@user2.balance += amount_default_currency * @user2.currency.exchange_rate.to_d	
	
    @user.save
	@user2.save

    if @user2.owner_id != 0
      ruser = User.where(:id => @user2.owner_id).first
      ruser.balance += amount_default_currency * ruser.currency.exchange_rate.to_d
      ruser.save
      pr = Payment.new({:gross => amount_default_currency, :paymenttype => "balance_transference", :amount => @amount, :currency => @user.currency.name, :date_added => Time.now, :shipped_at => Time.now, :completed => 1, :user_id => ruser.id, :first_name => ruser.first_name, :last_name => ruser.last_name})
      pr.save
    end
    #payment
    p1 = Payment.new
    p1.gross = @amount_to_extract * count_exchange_rate(@user.currency, session[:default_currency])
    p1.paymenttype = "balance_transference"
    p1.amount = - @amount_to_extract
    p1.currency = @user.currency.name
    p1.date_added = Time.now
    p1.shipped_at = Time.now
	p1.fee = 0
    p1.completed = 1
    p1.user_id = @user.id
    p1.first_name = @user.first_name
    p1.last_name = @user.last_name
    p1.owner_id = @user.owner_id
    p1.save

		
	p2 = Payment.new
    p2.gross = amount_default_currency
    p2.paymenttype = "balance_transference"
    p2.amount = @amount
    p2.currency = @user.currency.name
    p2.date_added = Time.now
    p2.shipped_at = Time.now
	p2.fee = 0
    p2.completed = 1
    p2.user_id = @user2.id
    p2.first_name = @user2.first_name
    p2.last_name = @user2.last_name
    p2.owner_id = @user2.owner_id
    p2.save
	
	if !@user2.address.mob_phone.blank?
	  #START ----- Send Notification by SMS
	  @user1 = User.where(:id => 0).first
	  message = _('Body_message_new_payment', p2.paymenttype.to_s, p2.amount.to_s, @user2.username.to_s, nice_number(@user2.balance.to_d).to_s)
	  sms = SmsMessage.new
	  sms.sending_date = Time.now
	  sms.user_id = @user1.id
	  sms.reseller_id = @user1.owner_id
	  sms.number = @user2.address.mob_phone
	  sms.sms_type = "payment"
	  sms.reg_owner = @user2.owner_id
	  sms.save
	  if Confline.get_value("Send_fake_SMS_for_mobile_validation", @user2.owner_id).to_i == 0
		Thread.new {
	...
	if !@user2.email.blank?
		begin
				a = Thread.new {
				  send_email_to_user = EmailsController.send_user_email_after_payment(@user2, {:balance_transferrer => @user.username, :payment_amount => p2.amount, :payment_date => p2.shipped_at, :payment_fee => p2.fee, :payment_currency => p2.currency, :payment_type => p2.paymenttype})
				}
		rescue Exception => e
			  Action.add_action(@user2.id, "Balance_transference", "Email not sent to user #{@user2.username} after payment: #{p2.amount} #{p2.currency}")
		end
	else
			 Action.add_action(@user2.id, "Balance_transference", "User #{@user2.username} was not emailed after payment: #{p2.amount} #{p2.currency}. Email for is not registered.")
	end
    flash[:status] = _('Balance_transference_was_successful') +". " + @amount.to_s + " " + @user.currency.name.to_s + " - > " + @user2.username.to_s
...
=end
	...
	Confline.set_value("Allow_registration_only_passwords_in_devices", params[:allow_registration_only_passwords_in_devices].to_i)	#Adesh
	Confline.set_value("Send_fake_SMS_for_mobile_validation", params[:send_fake_SMS_for_mobile_validation].to_i, 0)	#Adesh
...
=end
  ...
  def settings_payments
	...
	@spaycurr = Confline.get_value("SafetyPay_Default_Currency")
	@mercadopagocurr = Confline.get_value("MercadoPago_Default_Currency")
  ...
  def settings_payments_change
	...
	# MercadoPago	
	mp_min = params[:mercadopago_min_amount].to_d
    mp_max = params[:mercadopago_max_amount].to_d
    mp_default = params[:mercadopago_default_amount].to_d
    if mp_default >= mp_min and mp_default <= mp_max and mp_max != 0
      default_amount = params[:mercadopago_default_amount]
    elsif mp_default > mp_max and mp_max != 0
      default_amount = params[:mercadopago_max_amount]
    elsif mp_default < mp_min
      default_amount = params[:mercadopago_min_amount]
    elsif mp_max == 0
      default_amount = params[:mercadopago_default_amount]
    else
      flash[:notice] = _("MercadoPago_default_amount_not_correct")
      redirect_to :action => 'settings_payments' and return false
    end
	
    Confline.set_value("MercadoPago_Enabled", params[:mercadopago_enabled].to_i)
    #Confline.set_value("MercadoPago_Disable_For_Reseller", params[:mercadopago_disable_for_reseller].to_i)
	Confline.set_value("MercadoPago_Client_id", params[:mercadopago_client_id])
	Confline.set_value("MercadoPago_Client_secret", params[:mercadopago_client_secret])
    Confline.set_value("MercadoPago_Default_Currency", params[:mercadopago_default_currency])
    Confline.set_value("MercadoPago_Transfer_Fee_Percent", params[:mercadopago_transfer_fee_percent])
    Confline.set_value("MercadoPago_Default_Amount", default_amount)
    Confline.set_value("MercadoPago_Min_Amount", params[:mercadopago_min_amount])
    Confline.set_value("MercadoPago_Max_Amount", params[:mercadopago_max_amount])
	Confline.set_value("MercadoPago_Test", params[:mercadopago_test])	
	Confline.set_value("MercadoPago_Frontend_Message", params[:mercadopago_frontend_message])
    ...
    #Adesh. SafetyPay
	spay_min_amount = params[:safetypay_min_amount].to_d
	spay_default_amount = params[:safetypay_default_amount].to_d
	if  spay_default_amount < spay_min_amount
      spay_default_amount = spay_min_amount
	end
    Confline.set_value("SafetyPay_Enabled ", params[:safetypay_enabled].to_i)
    Confline.set_value("SafetyPay_Lang ", params[:safetypay_lang].to_s)
    Confline.set_value("SafetyPay_Default_Currency", params[:safetypay_default_currency].to_s)
	Confline.set_value("SafetyPay_Default_Amount", spay_default_amount)	
    Confline.set_value("SafetyPay_Min_Amount", spay_min_amount)
	Confline.set_value("SafetyPay_Fee_Percent", params[:safetypay_fee_percent].to_d)
	Confline.set_value("SafetyPay_Test", params[:safetypay_test].to_i)	  
    Confline.set_value("SafetyPay_Key", params[:safetypay_key].to_s)
    Confline.set_value("SafetyPay_Signature", params[:safetypay_signature].to_s)
	Confline.set_value("SafetyPay_Frontend_Message", params[:safetypay_frontend_message].to_s)
    # Adesh. /SafetyPay
	
	#Adesh. Baloto-Citibank (a manual payment)
    Confline.set_value("BalotoCiti_Enabled ", params[:balotociti_enabled].to_i)
	Confline.set_value("BalotoCiti_Frontend_Message", params[:balotociti_frontend_message].to_s)
    # Adesh. /Baloto-Citibank
...
=end
  ...
  def reseller_settings_change
    ...
    Confline.set_value("Send_fake_SMS_for_mobile_validation", params[:send_fake_SMS_for_mobile_validation].to_i, session[:user_id])	
